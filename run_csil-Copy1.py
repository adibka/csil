...
# Writes ONE "progress.csv" row per *epoch* (i.e., every --eval_every steps).
# Aggregates evaluator episode metrics (mean/std across --evaluation_episodes)
# and aggregates training metrics (mean over writes within the epoch).
...
class _OnlineStat:
  ...
  def get(self):
    if self.n <= 1: return (self.mean if self.n else 0.0), 0.0, self.n
    var = self.M2 / (self.n - 1)
    return self.mean, var ** 0.5, self.n
...
class _ProgressState:
  ...
  def __init__(self, csv_path: str, eval_every: int, episodes_per_eval: int):
    ...
    self.eval_every = int(max(1, eval_every))
    self.episodes_per_eval = int(max(1, episodes_per_eval))
    self.lock = _threading.Lock()
    self.epoch_idx = 0
    self.next_flush_step = self.eval_every
    # Per-epoch accumulators
    self.train_stats = defaultdict(lambda: defaultdict(_OnlineStat))     # label -> key -> stat
    self.eval_stats  = defaultdict(lambda: defaultdict(_OnlineStat))     # label -> key -> stat  (episode-level)
    self.eval_episode_counts = defaultdict(int)                          # label -> episodes collected this eval
    self.latest_steps = {}  # steps_key -> value
    # header columns order
    self.header = None
    Path(csv_path).parent.mkdir(parents=True, exist_ok=True)
    if not Path(csv_path).exists():
      with open(csv_path, "w", newline="") as f:
        _csv.writer(f).writerow([])  # placeholder until we know metrics

  def _maybe_flush_locked(self, step_now: int):
    # Flush only at eval cadence OR when full set of eval episodes collected
    should = (step_now >= self.next_flush_step)
    eval_ready = any(c >= self.episodes_per_eval for c in self.eval_episode_counts.values())
    if not should and not eval_ready:
      return
    # Build a row
    row = OrderedDict()
    row["time/iso"] = _dt.datetime.now().isoformat(timespec="seconds")
    # Prefer actor_steps if present, else learner_steps
    if "actor_steps" in self.latest_steps:
      row["global/actor_steps"] = int(self.latest_steps["actor_steps"])
    elif "learner_steps" in self.latest_steps:
      row["global/learner_steps"] = int(self.latest_steps["learner_steps"])
    row["Epoch"] = int(self.epoch_idx)

    # Training: mean over the epoch
    for label, m in sorted(self.train_stats.items()):
      for key, stat in sorted(m.items()):
        if key in {"step","steps","actor_steps","learner_steps","evaluator_steps"}:
          continue
        mean, std, n = stat.get()
        row[f"{label}/{key}"] = mean

    # Evaluation: aggregate episode metrics across the eval window
    # We record per-episode values in update(); here we compute mean/std
    def _emit_eval_stat(key_in, col_mean, col_std=None):
      # Merge across any evaluator labels we saw
      agg = _OnlineStat()
      for label in self.eval_stats:
        st = self.eval_stats[label].get(key_in)
        if st:
          # We don't have raw values to merge; but our OnlineStat objects were filled with raw episode values already.
          # So just take the first; all labels share same OnlineStat across the epoch.
          pass
      # grab from first available label
      for label in self.eval_stats:
        if key_in in self.eval_stats[label]:
          s = self.eval_stats[label][key_in]
          mean, std, n = s.get()
          row[col_mean] = mean
    ...
    _emit_eval_stat("episode_return", "evaluation/episode_return_mean", "evaluation/episode_return_std")
    _emit_eval_stat("success", "evaluation/success_rate")
    _emit_eval_stat("episode_length", "evaluation/episode_length_mean", "evaluation/episode_length_std")
    _emit_eval_stat("imitation_return", "evaluation/imitation_return_mean", "evaluation/imitation_return_std")
    _emit_eval_stat("episode_reward_corr", "evaluation/episode_reward_corr_mean", "evaluation/episode_reward_corr_std")

    # Prepare header if first real row
    if self.header is None or not any(self.header):
      self.header = list(row.keys())
      with open(self.csv_path, "w", newline="") as f:
        _csv.writer(f).writerow(self.header)

    # Append row in header order; extend header if new keys appear
    if set(row.keys()) - set(self.header):
      self.header += [k for k in row.keys() if k not in self.header]
    ...
    self.epoch_idx += 1
    self.next_flush_step += self.eval_every
    self.train_stats = defaultdict(lambda: defaultdict(_OnlineStat))
    self.eval_stats  = defaultdict(lambda: defaultdict(_OnlineStat))
    self.eval_episode_counts = defaultdict(int)

  def update(self, label: str, steps_key: str, data: dict, eval_labels=("imitation_evaluator","video_evaluator","bc_evaluator","evaluator","evaluation")):
    # Update latest seen step
    step_val = (data.get(steps_key) or data.get("actor_steps") or data.get("learner_steps") or data.get("evaluator_steps"))
    if step_val is not None:
      try: self.latest_steps[steps_key] = int(step_val)
      except Exception: pass

    # Decide if eval stream
    is_eval = any(lbl in label for lbl in eval_labels)

    # Helper: accept numeric scalars only
    def _num(x):
      import numbers
      try:
        ...
        if hasattr(x, "item"): 
          xi = x.item()
          if isinstance(xi, numbers.Number): return float(xi)
      except Exception: pass
      return None

    if is_eval:
      # evaluator logs per-episode metrics we care about
      for k,v in data.items():
        if k in {"timestamp", steps_key, "actor_steps","learner_steps","evaluator_steps","steps","step"}:
          continue
        xv = _num(v)
        if xv is None: 
          continue
        self.eval_stats[label][k].add(xv)
      if ("episode_return" in data) or ("success" in data):
        self.eval_episode_counts[label] += 1
    else:
      for k,v in data.items():
        if k in {"timestamp", steps_key, "actor_steps","learner_steps","evaluator_steps","steps","step"}:
          continue
        ...
        if xv is None: 
          continue
        self.train_stats[label][k].add(xv)

    # maybe flush
    step_now = 0
    if "actor_steps" in self.latest_steps:
      step_now = int(self.latest_steps["actor_steps"])
    elif steps_key in self.latest_steps:
      step_now = int(self.latest_steps[steps_key])
    with self.lock:
      self._maybe_flush_locked(step_now)
...
class ProgressCSVLogger:
  ...
  def write(self, data: dict):
    ...
    try:
      ...
      try: print(f"[ProgressCSVLogger:{self._label}] logging error: {e}")
      except Exception: pass
  def close(self): pass
